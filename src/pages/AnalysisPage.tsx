...
export const AnalysisPage: React.FC = () => {
  ...
  const schema = (location.state && (location.state as any).schema) || (typeof window !== 'undefined' ? localStorage.getItem('schema') && JSON.parse(localStorage.getItem('schema') as string) : undefined);
  ...
  // Ref for the query section
  const querySectionRef = useRef<HTMLDivElement>(null);

  // Scroll to the query section when the page loads
  useEffect(() => {
        ...
        >
          <div className={`text-center mb-8 ${isFullScreen ? 'mb-12' : ''}`}>
            ...
            <p className={`text-lg md:text-xl mb-8 max-w-3xl mx-auto leading-relaxed ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Comprehensive analysis of your SQL query with performance insights and optimization recommendations.
            </p>
          </div>
